...
function send(r,s,cmd){if(r.phase==='action'&&(cmd.type==='tablePlace'&&cmd.zone==='action'||['resolveTable','pass'].includes(cmd.type)))act(r,r.players[s].token,{version:r.version,type:'beginBattle'},cards);return act(r,r.players[s].token,{version:r.version,...cmd},cards)}
function start(r){r.phase='order';r.orderWinner=0;send(r,0,{type:'chooseOrder',first:true});send(r,0,{type:'ready'});if(r.status==='waiting')send(r,1,{type:'ready'});if(r.phase==='draw')send(r,r.active,{type:'draw'});return r}
...
test('Bot-selected first order never leaves setup waiting for an impossible human action',()=>{for(let i=0;i<20;i++){const r=setup(true);if(r.first===1){assert.equal(r.players[1].ready,true);assert.equal(r.setupSeat,0);send(r,0,{type:'ready'});assert.equal(r.status,'playing');assert.ok(['draw','defense','action'].includes(r.phase));return}}});
...
test('Setup order, unrestricted one-time mulligan, hidden leader, first draw 1 then 2',()=>{const r=setup();r.orderWinner=0;send(r,0,{type:'chooseOrder',first:true});assert.deepEqual(publicRoom(r,r.players[1].token).players[0].field,[]);assert.throws(()=>send(r,1,{type:'mulligan',indices:[0]}));send(r,0,{type:'mulligan',indices:[0,1,2,3,4]});assert.equal(r.players[0].hand.length,5);assert.throws(()=>send(r,0,{type:'mulligan',indices:[]}));send(r,0,{type:'ready'});send(r,1,{type:'ready'});assert.equal(r.phase,'draw');assert.equal(r.players[0].hand.length,5);assert.throws(()=>send(r,1,{type:'draw'}));send(r,0,{type:'draw'});assert.throws(()=>send(r,0,{type:'draw'}));assert.equal(r.players[0].hand.length,6);assert.equal(r.players[1].hand.length,5);send(r,0,{type:'pass'});assert.equal(r.turn,2);assert.equal(r.phase,'draw');assert.equal(r.players[1].hand.length,5);send(r,1,{type:'draw'});assert.equal(r.players[1].hand.length,7);conserve(r)});
...
test('Refresh empty deck from trash; deck and trash both empty loses',()=>{const r=start(setup());const p=r.players[1];p.trash.push(...p.deck.splice(0));send(r,0,{type:'pass'});send(r,1,{type:'draw'});assert.ok(p.deck.length>0);assert.equal(r.status,'playing');const r2=start(setup());r2.players[1].hand.push(...r2.players[1].deck.splice(0));send(r2,0,{type:'pass'});if(r2.status!=='finished')send(r2,1,{type:'draw'});assert.equal(r2.status,'finished');assert.equal(r2.winner,0)});
...
test('Full starter bot game advances with effects and conserves all cards',()=>{
 const r=start(setup(true));for(let step=0;step<600&&r.status!=='finished';step++){
  ...
  else if(r.phase==='draw')send(r,0,{type:'draw'});
  else if(r.phase==='action'||r.phase==='battle'||r.phase==='defense'){
 ...
 for(const a of palette)for(const b of palette)for(const pursuit of [0,1]){
  ...
  const go=(seat,cmd)=>{if(r.phase==='action'&&cmd.type==='resolveTable')act(r,r.players[seat].token,{version:r.version,type:'beginBattle'},custom);return act(r,r.players[seat].token,{version:r.version,...cmd},custom)};
...
test('Main permits preparation only; Battle forbids charge, leader switch and level',()=>{
 const r=start(setup()),p=r.players[0];const raw=cmd=>act(r,p.token,{version:r.version,...cmd},cards);
 assert.equal(r.phase,'action');
 assert.throws(()=>raw({type:'tablePlace',zone:'action',index:0,code:p.hand[0],x:0,y:0}));
 assert.throws(()=>raw({type:'resolveTable'}));
 raw({type:'tablePlace',zone:'concerto',index:0,code:p.hand[0],x:0,y:0});
 raw({type:'beginBattle'});assert.equal(r.phase,'battle');
 assert.throws(()=>raw({type:'tablePlace',zone:'concerto',index:0,code:p.hand[0],x:0,y:0}));
 assert.throws(()=>raw({type:'leader',code:p.field.find(x=>x!==p.leader)}));
