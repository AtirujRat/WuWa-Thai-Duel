...
export function resolveTraining(a, b) {
  ...
  if (!a || !b) {
    ...
    winner = beats[a.color] === b.color ? 0 : 1;
    reason = `${winner === 0 ? a.color : b.color} ชนะ ${winner === 0 ? b.color : a.color}`;
  }
  return {
    winner,
...
export function botAction(room, seat, cmd, cards) {
  ...
  if (cmd.type === "surrender") {
    ...
    return botAction(
      room,
      seat,
      { type: "commit", index: me.hand.length - 1, version: cmd.version },
      cards,
    );
    ...
    if (me.hp === 0 || bot.hp === 0 || room.turn >= 40) {
      room.status = "finished";
      room.winner = me.hp === bot.hp ? null : me.hp > bot.hp ? 0 : 1;
      log(
        room,
        room.winner === null
          ? "จบเกม · เสมอ"
