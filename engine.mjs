import {initRules,rulesAct,rulesView,isBotPending,getBotStepDelay,stepBot} from './rules-core.mjs';
...
export function makePlayer(name,entries,cards){const v=validateDeck(entries,cards);if(!v.valid)throw Error(v.errors.join(' · '));const list=Object.entries(entries).flatMap(([code,n])=>Array(n).fill(code));const map=new Map(cards.map(c=>[c.code,c]));const characters=list.filter(c=>map.get(c).type==='character');const draw=shuffle(list.filter(c=>map.get(c).type==='action'));const field=characters.filter(c=>map.get(c).level==='0');return {token:key(),name:name.trim().slice(0,32)||'ผู้เล่น',hp:20,ready:false,hand:draw.splice(0,5),deck:draw,field,reserve:characters.filter(c=>!field.includes(c)),leader:field[0],stacks:Object.fromEntries(field.map(c=>[c,[c]])),table:[],action:[],concerto:[],trash:[],pending:null,mulligan:false,revealHand:false};}
export function publicRoom(room,token){const seat=room.players.findIndex(p=>p.token===token);if(seat<0)throw Error('คุณไม่มีสิทธิ์เข้าห้องนี้');return {...(room.rulesVersion?rulesView(room,seat):{}),mode:room.mode||'friend',difficulty:room.difficulty,botPhase:room.botPhase,lastDuel:room.lastDuel,winner:room.winner,code:room.code,version:room.version,turn:room.turn,active:room.active,status:room.status,seat,log:room.log,players:room.players.map((p,i)=>({name:p.name,hp:p.hp,energy:p.energy||0,isBot:!!p.isBot,ready:p.ready,leader:room.rulesVersion&&room.status==='waiting'&&i!==seat?null:p.leader,field:room.rulesVersion&&room.status==='waiting'&&i!==seat?[]:p.field,stacks:room.rulesVersion&&room.status==='waiting'&&i!==seat?{}:(p.stacks||Object.fromEntries((p.field||[]).map(c=>[c,[c]]))),used:p.used,locked:p.locked,flags:i===seat?p.flags:undefined,table:(p.table||[]).map(c=>({...c,code:c.faceDown&&i!==seat?null:c.code})),action:p.action,concerto:p.concerto,trash:p.trash,handCount:p.hand.length,deckCount:p.deck.length,reserveCount:p.reserve.length,hand:i===seat||p.revealHand||room.players[seat].flags?.peek?p.hand:[],reserve:i===seat?p.reserve:[],pending:i===seat?p.pending:!!p.pending,mulligan:p.mulligan,revealHand:p.revealHand}))};}
export function act(room,token,cmd,cards,options={}){const seat=room.players.findIndex(p=>p.token===token);if(seat<0)throw Error('คุณไม่มีสิทธิ์เข้าห้องนี้');if(room.rulesVersion){rulesAct(room,seat,cmd,cards,options);return publicRoom(room,token)}if(room.mode==='bot'&&!['ready','mulligan','tablePlace','tableMove','tableFlip','tableTake'].includes(cmd.type)){botAction(room,seat,cmd,cards);return publicRoom(room,token)}const p=room.players[seat],map=new Map(cards.map(c=>[c.code,c]));let note='';const live=()=>{if(room.status!=='playing')throw Error('รอผู้เล่นทั้งสองกดพร้อมก่อน')};const move=(from,to,index)=>{if(!Array.isArray(p[from])||!Array.isArray(p[to])||!Number.isInteger(index)||index<0||index>=p[from].length)throw Error('ไม่พบการ์ด');const c=p[from].splice(index,1)[0];p[to].push(c);return c};
